describe('empty spec', () => {
  it('passes', () => {
    ...
    cy.get("input[name='xjudul']").clear().type('boneka');
    cy.get('input[type=file]').selectFile('cypress/e2e/images/boneka.jpg')
    cy.get('#cke_1_contents > iframe').then($element => {

      const $body = $element.contents().find('body')
    
      let stripe = cy.wrap($body)
      stripe.find('p').eq(0).click().clear().type('boneka kucing dengan kostum hiu')
    })
    cy.wait(2000)
